...
uint8_t MRT_getContinuity(void){
	uint8_t drogue = HAL_GPIO_ReadPin(IN_EJ_Drogue_Cont_GPIO_Port, IN_EJ_Drogue_Cont_Pin);
	uint8_t main = HAL_GPIO_ReadPin(IN_EJ_Main_Cont_GPIO_Port, IN_EJ_Main_Cont_Pin);
	...
	uint8_t continuity = drogue + (main*2)+ (prop1*4) + (prop2*8);
	return continuity;
}
