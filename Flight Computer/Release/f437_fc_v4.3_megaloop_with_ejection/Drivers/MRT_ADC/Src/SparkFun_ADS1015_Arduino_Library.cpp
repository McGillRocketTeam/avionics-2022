...
uint16_t ADS1015::readRegister(uint8_t location)
{
  ...
  int one = _i2cPort->read();
  int onem = one << 8;
  int two = _i2cPort->read();
  int val = onem | two;
...
//TODO homemade functions

uint8_t ADS1015::MRT_adc1015_setup(uint8_t i2c_bus, void (*adc1015_print)(char*)){
	this->print = adc1015_print;

	this->print((char*) "\r\nADC1015 Init\r\n");

	this->print((char*) "\tSetting I2C bus...");
	if (i2c_bus == 1){
		this->begin(0x48, Wire);
	}
	else if (i2c_bus == 2){
		this->begin(0x48, Wire1);
	}
	else if (i2c_bus == 3){
		this->begin(0x48, Wire2);
	}
	this->print((char*) "OK\r\n");
	...
	if(connected){
		...
		this->print((char*) "\tError: ADC1015 failed to connect. Check I2C connection\r\n");
		return -1;
	}

	this->print((char*) "Setting the gain...");
	this->setGain(ADS1015_CONFIG_PGA_4); //Max voltage of 120 mV, gain for +/-256 mV range
	this->print((char*) "OK\r\n");


	this->print((char*) "Setting the mode...");
	//this->setMode(ADS1015_CONFIG_MODE_CONT); //Setting mode to continuous to get more readings (should change when landed)
	this->print((char*) "OK\r\n");

	this->print((char*) "Setting the sample rate...");
	//this->setSampleRate(ADS1015_CONFIG_RATE_1600HZ); //Default (1 is slowest, 7 is fastest)
	this->print((char*) "OK\r\n");
}
...
uint16_t ADS1015::MRT_getDifferential(){
	return this->getDifferential(ADS1015_CONFIG_MUX_DIFF_P0_N1); //A0 and A1 differential
}

/*
uint16_t ADCS1015::MRT_getSingleEnded(){
	return this->getSingleEnded
}
*/
