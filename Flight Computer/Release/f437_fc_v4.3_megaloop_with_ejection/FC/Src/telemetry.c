...
//	tank_pressure = convert_prop_tank_pressure(); // convert buffered readings to voltage
	...
	#ifdef NEW_ADC //TODO
	tank_pressure = hadc1015.getDifferential();
	/*
	uint16_t a0 = hadc1015.getSingleEnded(0);
	uint16_t a1 = hadc1015.getSingleEnded(1);
	tank_pressure = a0 - a1;
	*/
	#else
	tank_pressure = tank_pressure_buf[PROP_TANK_PRESSURE_ADC_BUF_LEN-1];
	#endif
