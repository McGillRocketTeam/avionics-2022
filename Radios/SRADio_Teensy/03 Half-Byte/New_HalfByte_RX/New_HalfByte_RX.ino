...
uint8_t* decoded_buffer = (uint8_t*)malloc(DECODE_BUFFER_SIZE);
uint8_t len_decoded_buffer = 0;
...
void setup() {
  ...
  memset(decoded_buffer, 0, DECODE_BUFFER_SIZE);
  ...
  if ( irq_status & SX1262_IRQ_TIMEOUT ) {
    ...
    if ((irq_status & SX1262_IRQ_HEADER_ERR) || (irq_status & SX1262_IRQ_CRC_ERR)) {
      ...
      if (command_status != COMMAND_SUCCESS) {
        ...
        hb_decode_string(received_data, DATA_SIZE, decoded_buffer, &len_decoded_buffer);
        decoded_buffer[len_decoded_buffer] = '\0';//null terminate string for print
        Serial.println((char*)decoded_buffer);
        memset(received_data, 0, DATA_SIZE);        
        memset(decoded_buffer, 0, len_decoded_buffer);
      }
    }
  }
}
