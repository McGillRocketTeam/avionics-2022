...
char data_string[] = "0123456789-.,PSE"; // 16 characters
uint8_t string_length = sizeof(data_string) - 1;//string is null terminated but we dont want to encode the terminator


//half byte encoder buffers
#define ENCODE_BUFFER_SIZE 256
uint8_t* encoded_buffer = (uint8_t*)malloc(ENCODE_BUFFER_SIZE);
uint8_t len_encoded_buffer = 0;
...
void setup() {
  ...
  memset(encoded_buffer, 0, ENCODE_BUFFER_SIZE);
}

void loop() {
  delay(deley);
  
  hb_encode_string(data_string, string_length, encoded_buffer, &len_encoded_buffer);  
    
  Serial.println((char*)encoded_buffer);  
  
  device.clearIrqStatus(SX1262_IRQ_TX_DONE | SX1262_IRQ_TIMEOUT);
  device.writeBuffer(0, encoded_buffer, len_encoded_buffer);
  device.setLoRaPacketParams(12, 0, len_encoded_buffer, 1, 0);
  device.setTx(0x061A80);

  memset(encoded_buffer, 0, len_encoded_buffer);
