...
command_status_t SX1262::writeCommand(uint8_t opcode, uint8_t *cmd_buffer, uint16_t cmd_len) {
    ...
    if( /*((cmd_status & 0b00001110) == SX1262_STATUS_CMD_TIMEOUT) ||*/
        ...
        (cmd_status == 0xFF)) {
            ...
            Serial.print("Command status is: ");
...
command_status_t SX1262::readBufferUnknownLength(uint8_t *received_buf) {
    uint8_t rx_status[2] = {0};
    getRXBufferStatus(rx_status); // Update size (rx_buffer[0]) and offset (rx_buffer[1]) for new packet

    uint16_t buf_size = rx_status[0]+2;
    uint8_t cmd_buf[buf_size] = {SX1262_CMD_NOP};
    cmd_buf[0] = rx_status[1];
    return readCommand(SX1262_READ_BUFFER, cmd_buf, buf_size, received_buf, 2);

}

command_status_t SX1262::readBufferKnownLength(uint8_t offset, uint8_t data_size, uint8_t *received_buf) {
    if (data_size > 254) {
        return COMMAND_BUFFER_ERROR;
    }
    uint16_t buf_size = data_size+2;
    uint8_t cmd_buf[buf_size] = {SX1262_CMD_NOP};
    cmd_buf[0] = offset;
    return readCommand(SX1262_READ_BUFFER, cmd_buf, buf_size, received_buf, 2);
}
...
command_status_t SX1262::getPacketType(uint8_t *type) {
    ...
    return readCommand(SX1262_GET_PACKET_TYPE, cmd_buf, 2, type, 0);
}
...
command_status_t SX1262::getStatus(uint8_t *status) {
    ...
    return readCommand(SX1262_GET_STATUS, cmd_buf, 1, status,0);
}
...
command_status_t SX1262::getRssiInst(uint8_t *rssi) {
    uint8_t cmd_buf[2] = {SX1262_CMD_NOP};
    return readCommand(SX1262_GET_RSSI_INST, cmd_buf, 2, rssi, 0);
}

command_status_t SX1262::getStats(uint8_t *stats) {
    uint8_t cmd_buf[7] = {SX1262_CMD_NOP};
    return readCommand(SX1262_GET_STATS, cmd_buf, 7, stats, 1);
}
...
command_status_t SX1262::getDeviceErrors(uint16_t *device_errors) { // TODO Check THIS
    ...
    uint8_t cmd_buf[3] = {SX1262_CMD_NOP};
    command_status_t command_status = readCommand(SX1262_GET_DEVICE_ERRORS, cmd_buf, 3, read_buf, 1);
